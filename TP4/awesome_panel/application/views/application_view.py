...
class ApplicationView(pn.Template):
    ...
    main = param.ClassSelector(
        class_=pn.reactive.Reactive, allow_None=False, constant=True
    )
    sidebar = param.ClassSelector(
        class_=pn.reactive.Reactive, allow_None=False, constant=True
    )
    topbar = param.ClassSelector(
        class_=pn.reactive.Reactive, allow_None=False, constant=True
    )
    spinner = param.ClassSelector(
        class_=pn.reactive.Reactive, allow_None=False, constant=True
    )
    navigation = param.ClassSelector(
        class_=pn.reactive.Reactive, allow_None=False, constant=True
    )

    application_css = param.ClassSelector(
        class_=pn.pane.HTML, allow_None=False, constant=True
    )
    application_js = param.ClassSelector(
        class_=pn.pane.HTML, allow_None=False, constant=True
    )

    theme_css = param.ClassSelector(
        class_=pn.pane.HTML, allow_None=False, constant=True
    )
    ...
    def _get_navigation():
        return pn.Row(
            name="ApplicationComponent.navigation", sizing_mode="stretch_width"
        )

    @staticmethod
    def _get_spinner():
        return pn.Column(
            name="ApplicationComponent.spinner", sizing_mode="stretch_both"
        )
