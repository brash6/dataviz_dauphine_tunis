...
from TP4.utils.formatters import DATA_TABLE_FORMATTER, DYA_DATA_TABLE_FORMATTER
...
class DataTablePlot(BasePlot):
    ...
    def __init__(self, dict_formats, title, names=None, height=300, width=1200, **params):
        """
    ...
    def init_columns(self):
        ...
        for idx, val in enumerate(self.dict_formats):
            ...
            if self.formats[idx] == "1000seperator_round1":
                self.columns.append(
                    TableColumn(
                        field=val,
                        title=self.names[idx],
                        formatter=NumberFormatter(format="0,0.0", text_align="right"),
                    )
                )
